	...
	if($query && mysql_num_rows($query) > 0){
	  while($row = mysql_fetch_object($query)){
		...
		echo '<p><a name="'.$row->SumTanggal.'" id="'.$row->SumTanggal.'"></a>
          <img src="icon/arroworange.gif" width="12" height="11" /> 
          <a href="#'.$row->SumTanggal.'" onClick="loadArtikel(this,\''.$row->SumTanggal.'\')">'.$row->SumTanggal.'</a> ('.$row->TotalArtikel.')<span></span></p>';
	  }
	}
?>